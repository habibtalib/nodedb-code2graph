...
| SystemVerilog   | `.sv` `.svh`                            |   🟢   |   ✓   |    ✓    |    —    |    ✓     |     —      |           | hardware; module instantiations → type-refs; `` `include `` = file-level import (textual, like C's `#include`); no elaboration/parameterization semantics (deferred); generate blocks table-stakes only (walked, not expanded); Read/Write on module signals not emitted; class `extends` not emitted in v1 |
| Fortran         | `.f90` `.f`                             |   🟢   |   ✓   |    ✓    |         |          |            |           | modern (F90+) modules get real `public`/`private` visibility (bare `private` flips the module default; explicit lists override per name); `program`/`contains`-internal procedures are host-only → `Private`; legacy fixed-form `.f` dispatches but caps at whatever the grammar yields (no modules/`use`/visibility pre-F90); `name(args)` is syntactically ambiguous with array access, so some Call refs are array accesses (left unresolved); no generic-interface resolution |
| Groovy          | `.groovy` `.gradle`                     |   🟢   |   ✓   |    ✓    |    ✓    |    ✓     |     ✓      |     ✓     | `.gradle` parsed as plain Groovy — no Gradle-DSL semantics (dependency coordinates, task graph); `trait` keyword unsupported by tree-sitter-groovy 0.1.2 (parses as an unrelated call + closure — not extracted); paren-less calls captured only where the AST is unambiguous (`println "hi"`, not `println env`); `methodMissing`/dynamic dispatch unresolved; unmarked members are implicitly `Public` (opposite of Java) |
| Objective-C     | `.m` `.mm`                              |   🟢   |   ✓   |    ✓    |    ✓    |          |            |           | bare `.h` stays mapped to C (locked decision D-01) — ObjC declarations in headers are extracted as C facts, no content sniffing; methods use selector-form names (`compute:with:`); message sends carry the receiver as qualifier; categories are distinct `Base+Category` symbols; no enforced access control → ObjC symbols `Public`, C-level `static` → `Private`; dynamic dispatch (`performSelector:`) unresolved by design |
